...
//            #endif
        ...
        //  Sharpen Mask
        //  Only if we use texture input to define mixmapping
            #if defined(GEOM_TYPE_LEAF)
...
void Lux_SimplePOM (
    ...
    sampler2D heightmap )
{
//  TODO: slopeDamp leads to stretching at grazing angles 
    float slopeDamp = 1.0 - saturate (dot(lux.eyeVecTangent, float3(0,0,1)));
    //slopeDamp = saturate (dot(lux.eyeVecTangent, float3(0,0,1)));
...
//  We have to set mixmapValue (needed by surface shaders)
    #if !defined(GEOM_TYPE_LEAF)
        lux.mixmapValue = half2(lux.vertexColor.r, 1.0 - lux.vertexColor.r);
        float2 t_mixmapValue = lux.mixmapValue;
    #endif
    ...
    //  Adjust the mixmapValue ONLY when using Mask texture
        #if defined(GEOM_TYPE_LEAF)
    ...
    if(lux.detailBlendState > 0) {
        ...
        //  NOTE: Here we can actually do it! But not in the linear search!
                lux.mixmapValue = max( half2(0.0001, 0.0001), lux.mixmapValue * float2(dot(h0, lux.mixmapValue.x), dot(h1, lux.mixmapValue.y)));
            #if defined(GEOM_TYPE_LEAF)
                lux.mixmapValue *= lux.mixmapValue;
                lux.mixmapValue *= lux.mixmapValue;
    ...
    //  Adjust the mixmapValue when using vertex colors
    #if !defined(GEOM_TYPE_LEAF)
        //lux.mixmapValue = lerp(t_mixmapValue, lux.mixmapValue, lux.detailBlendState);
        //float2 blendVal = max( 0.0001, float2 ( dot(finalHeights.x, lux.mixmapValue.x), dot(finalHeights.y, lux.mixmapValue.y))) ;

// warum der ursprümnglciher wert????? 

        float2 blendVal = max( 0.0001, float2 ( dot(finalHeights.x, t_mixmapValue.x), dot(finalHeights.y, t_mixmapValue.y))) ;
        blendVal *= blendVal;
        blendVal *= blendVal;
        blendVal = blendVal / dot(blendVal, 1.0);
    //    lux.mixmapValue = blendVal;
        //lux.mixmapValue = lerp(lux.mixmapValue, blendVal, lux.detailBlendState);
        lux.mixmapValue = lerp(lux.mixmapValue , blendVal /*float2(1,0) */ , 1.0 - lux.detailBlendState);
